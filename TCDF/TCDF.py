...
from processEEGdata import Scores_Matrix
...
def findcauses(target, cuda, epochs, kernel_size, layers,
    ...
    X_train, Y_train, df_data = preparedata(file, target)
    ...
    print("X unsqueeze", X_train.shape)
    print("Y unsqueeze", Y_train.shape)
    '''
    ...
    loss_list = [file, firstloss]

    for ep in range(2, epochs + 1):
        scores, realloss = train(ep, X_train, Y_train, model, optimizer, log_interval, epochs)
        loss_list.append(realloss.cpu().data.item())

    with open('loss.csv', 'a', newline='') as loss_csv:
