...
import processEEGdata
from processEEGdata import save_result, Scores_Matrix
...
def plotgraph(datafile, alldelays, columns):
    ...
    G = nx.DiGraph()
    position = [(-1, 3), (0, 2), (-2, 2), (-4, 2), (-5, 1), (-3, 1), (-1, 1), (-2, 0), (-4, 0), (-5, -1), (-3, -1),
                (-1, -1),
                (0, -2), (-2, -2), (-4, -2), (-2, -3), (0, -3), (2, -3), (2, -2), (4, -2), (5, -1), (3, -1), (1, -1),
                (0, 0),
                (2, 0), (4, 0), (5, 1), (3, 1), (1, 1), (2, 2), (4, 2), (1, 3)]
    fixed_positions = {}
    for c, p in zip(columns, position):
        G.add_node(c)
        fixed_positions.setdefault(c, p)
    ...
    path = processEEGdata.result_save_path
    if not os.path.exists(path):
        os.makedirs(path)
    savefilename = savefilename[:-4] + "_TCDFresult.png"
...
def main(datafiles, evaluation):
    ...
    for datafile in datafiles.keys():
        ...
        save_result(alldelays, columns, datafile)
        Scores_Matrix(allscores, datafile)
        for pair in alldelays:
    ...
    with open('speed_time.csv', 'a', newline='') as speed_time:
        writer = csv.writer(speed_time)
        writer.writerow([stringdatafile, end_time-start_time])
...
if args.kernel_size != args.dilation_coefficient:
    print(
        "WARNING: The dilation coefficient is not equal to the kernel size. Multiple paths can lead to the same delays."
