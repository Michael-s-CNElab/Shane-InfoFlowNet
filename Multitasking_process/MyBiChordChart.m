...
%     values = linspace(cmin, cmax, numColors);
% 
%     for i = 1:numColors
%         % 计算颜色的R、G、B分量
%         normalizedValue = (values(i) - cmin) / (cmax - cmin); % 将值映射到0到1的范围
%         colorMap(i, :) = [normalizedValue, 0, 1-normalizedValue]; % 从蓝色到红色的渐变
%     end
% 
%     %% 自定義 colormap


    MBC.CData=colormap("jet");
    MBC.ax.NextPlot='add';
    MBC.dataMat=dataMat;
    MBC.ch_name=ch_name;
