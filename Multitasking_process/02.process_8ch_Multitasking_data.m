%% 8 channel pre-process
ch_name ={'Fz', 'F3', 'F4', 'Cz', 'C3', 'C4', 'Pz', 'Oz'};
data = ones(8, 1500, 30, 36);
RT = ones(30, 36);
bad_trial_info = {};
trial_count = 0;
trial_info = {};
...
for s=1:36
    ...
    for e=1:trials
        if strcmp(EEG_set{s, 1}.epoch(e).eventtype{end}, '2510')
            ...
            data(2, :, count, s) = Data(3, :);  %Fz
            data(3, :, count, s) = Data(30, :);  %Fz
            data(4, :, count, s) = Data(24, :); %Cz
            data(5, :, count, s) = Data(8, :);  %Fz
            data(6, :, count, s) = Data(25, :);  %Fz
            data(7, :, count, s) = Data(13, :); %Pz
            data(8, :, count, s) = Data(17, :); %Oz
